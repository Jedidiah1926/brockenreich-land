...
- 일반(인챈트되지 않은) 씨앗/모종/열매는 아예 심어지지 않습니다.
...
성장에 따라 텍스처가 바뀌는 작물(밀/당근/감자/비트/네더 와트/코코아/달콤한 열매/수박·호박 씨앗)은 심은 직후(0단계),
전체 시간의 1/3이 지났을 때(1단계), 2/3이 지났을 때(2단계), 다 자랐을 때(3단계) 이렇게 4단계로 실제 텍스처가
바뀌며 자랍니다 (버섯/나무 묘목은 단계별 텍스처가 따로 없는 종류라 심은 모습 그대로 있다가 다 자랄 때 한 번에
바뀝니다). 다 자라는 순간에는 뼛가루를 쓴 것과 같은 초록색 반짝임 파티클이 터집니다.
