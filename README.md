# BrockenreichLand

Paper 1.21.8 기반 마인크래프트 서버 플러그인 (Kotlin + Gradle).

## 요구 사항

- JDK 21
- VSCode + 권장 확장 (`.vscode/extensions.json` 참고: Kotlin, Gradle for Java, Extension Pack for Java)

## 빌드

```bash
./gradlew build
```

빌드된 플러그인 jar는 `build/libs/brockenreich-land-<version>.jar` 에 생성됩니다.

## 로컬 서버에서 바로 실행하기

이 저장소에는 플러그인 소스만 있고, 실제로 구동할 Paper 서버 본체(jar)는 포함되어 있지 않습니다.
아래 스크립트로 최신 Paper 1.21.8 빌드를 받아 `run/` 에 로컬 테스트 서버를 구성할 수 있습니다.

### Mac / Linux / Git Bash / WSL

```bash
# 1. Paper 1.21.8 서버 jar 다운로드 + run/ 구성 (Mojang EULA 동의 포함)
./scripts/setup-server.sh --accept-eula

# 2. 플러그인 빌드 후 run/plugins/ 에 자동 복사
./gradlew deployToRunServer

# 3. 서버 실행
./scripts/run-server.sh
```

### Windows (PowerShell)

`.sh` 스크립트는 PowerShell에서 실행되지 않으므로 `.ps1` 버전을 사용하세요.

```powershell
# 1. Paper 1.21.8 서버 jar 다운로드 + run/ 구성 (Mojang EULA 동의 포함)
.\scripts\setup-server.ps1 -AcceptEula

# 2. 플러그인 빌드 후 run/plugins/ 에 자동 복사
.\gradlew.bat deployToRunServer

# 3. 서버 실행
.\scripts\run-server.ps1
```

`실행할 수 없습니다. ... 이 시스템에서 스크립트를 실행할 수 없으므로...` 같은 실행 정책 오류가 나면,
아래 명령으로 현재 세션에서만 스크립트 실행을 허용한 뒤 다시 시도하세요.

```powershell
Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
```

### 공통 안내

- `--accept-eula` / `-AcceptEula` 없이 실행하면 `run/eula.txt` 가 `eula=false` 로 생성되며,
  https://aka.ms/MinecraftEULA 를 확인한 뒤 직접 `eula=true` 로 바꿔야 서버가 뜹니다.
- `run/` 은 `.gitignore` 에 포함되어 있어 서버 jar/월드 데이터가 커밋되지 않습니다.
- 서버를 종료하려면 콘솔에 `stop` 을 입력하세요.

## 구역(Area) 관리

WorldEdit(소프트 디펜던시)과 연동되는 땅 관리 기능입니다. `run/plugins/` 에 WorldEdit이 설치되어 있어야
`/area create` 로 새 구역을 만들 수 있습니다.

- **구역(region)**: WorldEdit으로 선택한 직육면체 영역에 이름을 붙인 것.
- **월드 기본 구역(world)**: 어떤 구역에도 속하지 않는, 월드의 나머지 모든 공간. `world:<월드이름>` 으로 지정.
- **멤버(member)**: 해당 구역에서 모든 권한을 항상 허용받는 플레이어 목록.
- **admin("땅 주인")**: 서버 OP가 아니어도, 그 구역 하나에 한해 `/area modify`/`/area info` 를 쓸 수 있는 플레이어 목록. member/권한과는 별개로, "이 명령을 쓸 수 있는가"에 대한 권한입니다. admin을 임명/해제하는 것 자체는 OP만 가능합니다 (admin이 스스로를 승격시키거나 다른 admin을 마음대로 못 늘리도록).
- **parent(부모 구역, admin 상속)**: 구역(region)에만 설정 가능. 어떤 구역에 부모 구역들을 지정하면, **모든 부모 구역에서 admin인 플레이어만** 자식 구역도 자동으로 admin이 됩니다 (AND 조건 — 부모가 여러 개면 전부 만족해야 함). 부모 중 하나에서라도 admin 자격을 잃으면 자식의 상속된 admin 자격도 즉시 같이 빠집니다. 순환 참조(자기 자신이 조상이 되는 구조)는 만들 수 없고, 설정도 OP 전용입니다.
- **소유 길드(ownerGuild)**: 구역(region)에만 설정 가능. 구역에 [길드](#길드guild-시스템)를 소유주로 지정하면, 그 길드의 **길드장과 간부**가 이 구역의 admin이 됩니다 (일반 길드원은 자동으로 admin이 되지 않음). parent와 마찬가지로 설정은 OP 전용입니다. 자세한 내용은 아래 길드 시스템 문단 참고.
- **`@everyone` 권한**: 멤버가 아닌 모든 플레이어에게 적용되는 기본 권한. 새 구역은 `administration` 을 제외한 모든 권한이 기본 허용 상태로 시작합니다.
- **개별 유저 권한**: 특정 닉네임에게 `@everyone` 설정과 별개로 추가로 권한을 허용할 수 있음 (`@everyone`이 막혀 있어도 개별 허용된 유저는 통과 가능 — 합집합 방식, 뺏는 방향으로는 동작하지 않음).

### 권한 목록

| 권한 | 설명 |
|---|---|
| `entrance` | 걸어서(또는 탈것 없이) 구역에 들어갈 수 있는가 |
| `exit` | 걸어서(또는 탈것 없이) 구역에서 나갈 수 있는가 |
| `boatEntrance` | 보트를 탄 채로 구역에 들어갈 수 있는가 (`entrance` 와 별개) |
| `boatExit` | 보트를 탄 채로 구역에서 나갈 수 있는가 (`exit` 와 별개) |
| `cartEntrance` | 마인카트를 탄 채로 구역에 들어갈 수 있는가 (`entrance` 와 별개) |
| `cartExit` | 마인카트를 탄 채로 구역에서 나갈 수 있는가 (`exit` 와 별개) |

보트가 경계선에 걸쳐있는 상태에서 내리면, 내린 자리 기준으로 걸어서 들어갈 수 있는 권한(`entrance`)이 없으면 내리는 것 자체가 막힙니다 (보트 이동 검사만으로는 못 잡는 경계 우회 방지). 마인카트는 이 하차 검사와, 보트에 있는 매 틱 보정(위치가 경계를 이미 넘어간 걸 되돌리는 로직)이 없는 최소 구현입니다 - 레일 위를 달리는 특성상 보트처럼 경계에 걸쳐 멈춰있는 경우가 훨씬 드물어서, 이동 중 경계를 넘으려 할 때 되돌리는 검사 하나로 충분합니다.

`entrance`/`exit`는 걸어서 이동하는 것뿐 아니라 엔더 진주 투척 및 코러스 열매 섭취로 인한 순간이동, 침대 사용(취침 위치가 경계 너머일 때)에도 동일하게 적용됩니다 (나가는 쪽 `exit`, 들어가는 쪽 `entrance` 모두 필요). 피스톤에 밀려서 경계를 넘는 경우처럼 이동 이벤트 자체가 발생하지 않는 케이스는 매 틱 위치를 검사해 허용되지 않은 위치면 직전의 안전한 위치로 되돌리는 별도의 보정 로직(AreaPlayerGuard)으로 막습니다.
| `interaction` | 상호작용 (버튼, 상자 등 우클릭 + 주민 거래, 동물 먹이주기, 아이템 액자 회전, 보트 탑승, 갑옷 거치대에 장비 넣고 빼기 등 엔티티 우클릭). 대상(블록/엔티티)의 위치 기준으로 검사하므로, 경계 바깥에서 사거리로 안쪽의 블록/엔티티를 상호작용하는 것도, 반대로 권한 없는 바깥 갑옷 거치대의 아이템을 권한 있는 안쪽에서 사거리로 빼가는 것도 막힙니다. 활공 중이 아닐 때 폭죽 로켓을 쏘는 것도 여기 포함 |
| `pickupItem` | 아이템 줍기 |
| `dropItem` | 아이템 던지기 |
| `blockBreak` | 블록 부수기 |
| `blockPlace` | 블록 설치하기 |
| `blockIgniting` | 블록에 불 붙이기 |
| `hangingPlace` | 액자/그림 등 걸이형 블록 설치 |
| `hangingBreak` | 액자/그림 등 걸이형 블록 파괴 |
| `projectileLaunch` | 발사체 발사 (활, 눈덩이 등). 엘리트라로 활공 중에 폭죽 로켓으로 부스트하는 것도 여기 포함 |
| `attackEntity` | 엔티티(몹 등) 공격 (근접 공격뿐 아니라 화살/삼지창 등 발사체로 맞히는 것도 포함). 공격자 위치와 피격자 위치 둘 다 검사하므로, 휩쓸기 공격이나 경계 밖에서 쏜 화살로 경계 너머의 엔티티를 맞히는 것도 막힙니다 |
| `attackPlayer` | 플레이어 공격. `attackEntity`와 동일하게 공격자/피격자 양쪽 위치를 검사하고, 발사체 공격도 포함합니다 |
| `bucketEmpty` | 양동이 비우기 |
| `bucketFill` | 양동이 채우기 |
| `fishPull` | 낚싯대로 엔티티/플레이어를 낚아서 당기기. 낚는 사람 위치와 낚인 대상 위치 둘 다 검사합니다 |
| `leash` | 목줄(끈)로 엔티티 묶기 |
| `administration` | 이 구역 안에서 다른 모든 권한 검사를 무시하는 와일드카드 권한 (기본값: 비허용) |

### 보호(Protection) 목록

권한이 "플레이어가 뭘 할 수 있는가"라면, 보호는 "기계 장치가 구역 경계를 넘을 수 있는가"를 막는 것으로, 기본값은 비활성(off)입니다.

| 보호 | 설명 |
|---|---|
| `piston` | 피스톤(스티키 피스톤 + 슬라임/꿀 블록으로 옆으로 끌려가는 블록 포함)이 이 구역의 경계 안팎으로 블록을 밀거나 당기지 못하게 막음 |
| `flood` | 이 구역 안에서 시작된 물/용암이 구역 경계 밖으로 퍼지지 못하게 막음 (외부에서 안으로 흘러들어오는 건 막지 않음) |
| `potion` | 투척용 포션이 이 구역 안에서 깨지면 파티클을 포함해 스플래시 자체가 통째로 취소됨. 구역 밖에서 깨진 경우엔 그 스플래시는 그대로 나되, 범위 안에서 이 구역에 있는 대상만 효과가 제외됨. 잔류형 포션도 이 구역 안에서 깨지면 잔류 구름 자체가 생기지 않고, 구역 밖에서 생긴 구름이 시간이 지나며 범위가 커져 안쪽까지 걸쳐도 그 순간 안에 있는 대상에게는 효과가 적용되지 않음 |
| `explosion` | TNT, 엔드 수정, 크리퍼, 위더, 침대/속박의 닻 등 모든 폭발로부터 이 구역 안의 블록이 파괴되지 않게 막음. 터지는 개체(TNT 등) 자체가 이 구역 안에 있으면 소리·파티클까지 포함해 폭발이 아예 통째로 취소됨. 개체는 밖에 있는데 폭발 범위만 구역 안까지 걸치는 경우엔, 이미 밖에서 소리·파티클이 발생한 뒤라 그건 그대로 나고 블록 파괴만 안쪽이 보호됨 |
| `explosionDamage` | 위 폭발들로 인해 이 구역 안에 있는 엔티티/플레이어가 데미지를 받지 않게 막음 (`explosion` 과 별개로 켜고 끌 수 있음) |
| `overflow` | 나무/거대버섯 등 구조물이 자랄 때, 묘목이 심어진 곳이 이 구역 안이면 나뭇잎 등 생성되는 블록이 구역 경계 밖으로 나가지 못하게 막음. 잔디에 뼛가루를 사용해 풀/꽃이 퍼지는 경우도 동일하게, 뼛가루를 사용한 블록이 이 구역 안이면 그 결과 블록이 경계 밖으로 나가지 못하게 막음 |
| `redstone` | 레드스톤 전선이 구역 경계와 맞닿아 있으면(이웃 블록이 다른 구역에 속하면) 신호 변화가 전달되지 않게 막음 — 경계 안쪽/바깥쪽 중 어느 쪽에 이 보호가 있어도 적용되어, 밖에서 온 신호가 안으로 못 들어오고 안에서 만든 신호도 밖으로 못 나감. 다만 관측기(observer)가 경계 너머의 블록 변화를 직접 감지하는 것은 이 보호로 막을 수 없음 (관측기의 감지는 레드스톤 전선 신호 전달과 다른 방식으로 동작해서, 바닐라 이벤트 상 구분해서 막을 방법이 없음) |
| `fire` | 이 구역 안에서 불이 붙지 못하게 막음 (부싯돌, 번개, 용암, 화염구 등 확산이 아닌 모든 발화 원인) |
| `fireSpread` | 이 구역 안에서 불이 스스로 번지지 못하게 막음 (인접 블록으로 불이 옮겨붙는 것 + 불에 타서 블록이 사라지는 것 둘 다) |
| `dispenser` | 발사기가 바라보는 방향 바로 앞 칸이 이 구역과 다른 구역이면 발사(화살/포션/불꽃 등)나 양동이 액체 분출을 막음. 안쪽을 향해 쏘는 것과 안쪽에서 안쪽으로 쏘는 것은 그대로 정상 작동, 경계를 넘나드는 경우만 막힘 (경계 안쪽/바깥쪽 중 어느 쪽에 이 보호가 있어도 적용됨). 화염구(화염구 조각으로 발사)는 날아가서 멀리 떨어진 곳에서 발화/폭발할 수 있어 이 한 칸 검사로는 못 막기 때문에, 발사기 자신이 있는 구역에 이 보호가 있으면 화염구 발사 자체를 아예 막음 |
| `sponge` | 스폰지가 이 구역 안에 있으면, 경계 밖에 있는 물은 빨아들이지 못하게 막음. 같은 구역 안의 물은 정상적으로 흡수됨 |
| `hopper` | 호퍼(또는 호퍼 미노카트)가 아이템을 다른 인벤토리로 옮기거나 바닥의 아이템을 줍는 것이, 옮기는 두 위치(호퍼 자신 ↔ 대상 인벤토리, 또는 호퍼 ↔ 바닥 아이템)가 서로 다른 구역에 속하면 막힘. 같은 구역 안에서의 이동은 정상 작동 (경계 안쪽/바깥쪽 중 어느 쪽에 이 보호가 있어도 적용됨) |
| `entityPickupItem` | 플레이어가 아닌 엔티티(여우, 주민 등)가 이 구역 안에서 떨어진 아이템을 줍지 못하게 막음 (플레이어의 아이템 줍기는 `pickupItem` 권한이 따로 담당) |
| `damage` | 이 구역 안에 있는 엔티티/플레이어가 낙사, 화상, 익사, 기아, 추락사 등 (플레이어/몹의 공격이나 폭발이 아닌) 모든 피해를 받지 않게 막는 안전지대 기능. `attackEntity`/`attackPlayer`(공격), `explosionDamage`(폭발)와는 독립적으로 작동 |
| `entityChangeBlock` | 엔티티로 인한 블록 변경(양이 잔디 뜯어먹기, 엔더맨의 블록 집기/놓기, 모래·자갈 등이 떨어져서 다시 블록으로 굳는 것, 위더/래바저의 블록 파괴 등)이 이 구역 안에서 일어나지 못하게 막음 |
| `blockSpread` | 잔디/균사체/덩굴/버섯/조개/스컬크 등이 퍼질 때, 퍼지기 시작한 블록이 이 구역 안이면 그 결과가 구역 경계 밖으로 나가지 못하게 막음 |
| `fade` | 얼음/눈이 녹거나 산호가 죽는 등, 자연적으로 사그라드는 현상이 이 구역 안에서 일어나지 못하게 막음 (경작지가 마르는 것은 이 서버 전역 규칙으로 이미 항상 막혀 있어 이 보호와 무관) |
| `form` | 얼음이 얼거나 눈이 쌓이거나 콘크리트가 굳는 등, 자연적으로 새로 생기는 현상이 이 구역 안에서 일어나지 못하게 막음 |
| `growth` | 작물이 자연적으로(또는 뼛가루로) 한 단계 자라는 것이 이 구역 안에서 일어나지 못하게 막음. 농장 시스템의 강제 성장도 내부적으로 뼛가루를 흉내 내는 방식이라 이 보호가 걸린 구역에서는 농장 작물도 다 자라지 못함 |

```
/area region create <이름>                             # 현재 WorldEdit 선택 영역으로 region:<이름> 생성
/area region delete <이름>
/area region rename <기존이름> <새이름>                 # 이미 존재하는 이름으로는 변경 불가
/area world create <월드이름>                           # world:<월드이름> 등록 (기본: administration 제외 모든 권한 허용)
/area world delete <월드이름>                           # world:<월드이름> 을 기본값으로 초기화
/area list
/area info <region:이름|world:월드이름>
/area modify <target> role add member <닉네임>
/area modify <target> role remove member <닉네임>
/area modify <target> role add admin <닉네임>          # OP 전용 - 이 구역만 관리할 수 있는 admin 임명
/area modify <target> role remove admin <닉네임>       # OP 전용
/area modify region:<이름> parent add <부모구역이름>    # OP 전용 - 모든 부모의 admin이면 자동 admin
/area modify region:<이름> parent remove <부모구역이름> # OP 전용
/area modify region:<이름> guild set <길드이름>         # OP 전용 - 길드장/간부가 이 구역의 admin이 됨
/area modify region:<이름> guild remove                # OP 전용
/area modify <target> role permission @everyone <add|remove> <권한>
/area modify <target> role permission <닉네임> <add|remove> <권한>
/area modify <target> protection <add|remove> <보호>
```

`member`/`admin`/`permission`은 전부 `role` 아래의 하위 종류입니다 — member도 admin도 결국 "역할"이라는 개념이고,
그 역할에 누구를 넣을지(member/admin) 또는 어떤 권한을 부여할지(permission)의 차이일 뿐입니다.

`region`/`world` 생성·삭제와 `list`, admin 임명/해제, parent 설정은 OP만 가능합니다. `modify`(role member/permission,
protection)와 `info`는 OP 이거나, 그 구역의 admin으로 등록된(또는 parent를 통해 상속받은) 플레이어면 자기 구역에
한해 사용할 수 있습니다.

parent 예시:
```
/area modify region:lime parent add white
/area modify region:lime parent add green
```
→ `white`와 `green` 양쪽 모두에서 admin인 플레이어만 `lime`도 자동으로 admin이 됩니다. 이후 그 플레이어가
`green`의 admin에서 제거되면, `lime`에 대한 admin 자격도 (별도 명령 없이) 즉시 같이 사라집니다.

역할(`@everyone`)은 항상 `@` 접두사로 지정하고, 특정 유저는 접두사 없이 닉네임을 그대로 씁니다.

예시:
```
/area region create green
/area modify region:green role add member Steve
/area world create world
/area modify world:world role permission @everyone remove entrance
/area modify world:world role permission Alex add entrance
/area modify region:green role permission @everyone remove blockBreak
```
→ `green` 구역은 `Steve`가 항상 모든 걸 할 수 있고, `world` 월드에서 구역으로 지정되지 않은 나머지 공간은
`@everyone`(월드의 멤버가 아닌 모든 플레이어)의 입장이 차단되지만, `Alex`는 개별로 입장이 허용되어
`@everyone` 설정과 무관하게 들어갈 수 있습니다. `green` 구역에서는 `@everyone`(멤버 제외)의 블록 파괴가 막힙니다.

전부 관리자(OP) 전용 명령이며, 설정은 `plugins/BrockenreichLand/areas.yml` 에 저장됩니다.

## 길드(Guild) 시스템

플레이어끼리 자율적으로 만드는 조직입니다. **길드는 법인과 비슷한 개념**이라, 길드 자체가 땅(구역)을 소유할
수 있습니다 — 특정 플레이어 한 명이 아니라 "회사" 명의로 부동산을 갖는 것과 같습니다. 실제로 그 땅을
관리(설정 변경)하는 건 회사의 대표/임원에 해당하는 **길드장·간부**이고, 일반 사원에 해당하는 **일반
길드원**은 길드에 속해 있다는 것만으로 그 땅의 관리 권한이 자동으로 생기지는 않습니다 (필요하면 구역 admin이
개별 멤버/권한으로 따로 허용해줘야 함).

### 직급

- **길드장**: 길드를 만든 사람. 간부 승급/강등, 길드 해체를 할 수 있음. 탈퇴는 불가 (해체만 가능).
- **간부**: 길드장이 승급시킨 사람. 길드원 초대/추방 가능.
- **일반 길드원**: 초대받아 들어온 사람. 초대/추방/승급 권한 없음.

### 명령어

```
/guild create <길드이름>                # 누구나 사용 가능, 본인이 길드장이 됨
/guild disband <길드이름>               # 길드장 전용 (OP는 예외적으로 항상 가능)
/guild invite <길드이름> <닉네임>        # 길드장/간부
/guild kick <길드이름> <닉네임>          # 길드장/간부 (간부는 길드장만 추방 가능)
/guild promote <길드이름> <닉네임>       # 길드장 전용 - 일반 길드원 → 간부
/guild demote <길드이름> <닉네임>        # 길드장 전용 - 간부 → 일반 길드원
/guild leave <길드이름>                 # 길드장 제외 누구나
/guild info <길드이름>
/guild list
```

### 구역과 연결하기

```
/area modify region:<이름> guild set <길드이름>   # OP 전용
/area modify region:<이름> guild remove           # OP 전용
```

구역(region)에만 설정할 수 있고(월드 기본 구역은 불가), 설정하는 순간부터 그 길드의 길드장·간부가 해당
구역의 admin이 됩니다 (`/area modify`/`/area info`를 그 구역에 한해 쓸 수 있음, 기존 admin 개념과 동일하게
작동 — 자세한 건 위 구역 관리 문단의 admin/parent 설명 참고). 길드에서 간부 자격을 잃으면 그 구역의 admin
자격도 즉시 같이 빠집니다. 설정은 `plugins/BrockenreichLand/guilds.yml` 에 저장됩니다.

## 칭호(Honor) 시스템

OP가 만들어서 플레이어에게 부여하는 칭호를 채팅/탭리스트/이름표(머리 위)에 표시하는 기능입니다. 명령어는
바닐라 `/title` 명령(액션바/타이틀 표시)과 이름이 겹치지 않도록 `/honor` 를 씁니다.

- **칭호 정의**: OP가 ID, 표시 텍스트(색상 코드 `&` 사용 가능), HEX 색상 코드로 칭호를 만듭니다. HEX
  코드가 표시 텍스트 전체의 기본 색이 됩니다.
- **부여**: OP가 특정 플레이어에게 칭호를 부여(grant)해야 그 플레이어가 장착할 수 있습니다. 부여받지 않은
  칭호는 장착 불가.
- **장착**: 부여받은 칭호 중 플레이어가 직접 하나를 골라 장착(equip)합니다. 장착한 칭호가 있으면 채팅
  메시지 앞과 탭리스트에 `[표시텍스트]` 형태로 붙고, 머리 위 이름표에도 같은 형태로 접두사로 붙습니다.
  장착한 칭호가 없으면 원래 이름(또는 닉네임) 그대로 나갑니다.
  - **머리 위 이름표 제약**: 마인크래프트 자체 한계로, 플레이어의 머리 위 이름표는 스코어보드 팀의
    접두사(prefix)로만 꾸밀 수 있고, 그 뒤에 오는 이름 부분은 언제나 실제 마인크래프트 계정 이름입니다
    (닉네임으로 못 바꿈 - 패킷 조작 없이는 불가능한 영역이라 이 플러그인은 손대지 않음). 즉 머리 위에는
    `[칭호] 실제계정이름` 형태로 뜨고, 채팅/탭리스트에서만 닉네임까지 완전히 반영됩니다.
- 칭호를 삭제하거나 회수하면, 그 칭호를 장착 중이던 플레이어는 자동으로 장착 해제됩니다.
- **강제 착용(force)**: OP가 특정 플레이어에게 칭호를 강제로 장착시키고 고정할 수 있습니다. 고정된
  동안에는 본인이 `/honor equip`/`/honor unequip` 을 써도 거부됩니다 (마음대로 빼거나 바꿀 수 없음).
  `/honor force <닉네임> none` 으로 "아무 칭호도 착용 못 하게" 고정할 수도 있습니다. `/honor unforce` 로
  풀어주면 본인이 다시 자유롭게 관리할 수 있습니다.

### 명령어

```
/honor create <ID> <표시텍스트...> <HEX코드>   # OP 전용, 표시텍스트에 & 색상 코드도 사용 가능 (예: #FFAA00)
/honor delete <ID>                              # OP 전용
/honor grant <닉네임> <ID>                      # OP 전용
/honor revoke <닉네임> <ID>                     # OP 전용
/honor force <닉네임> <ID|none>                 # OP 전용, 강제 착용시키고 본인이 못 바꾸게 고정
/honor unforce <닉네임>                         # OP 전용, 고정 해제
/honor equip <ID>                               # 본인이 부여받은 칭호만 장착 가능 (고정 중이면 불가)
/honor unequip
/honor mine                                     # 내가 보유한 칭호 목록 (장착 중인 것 표시)
/honor list                                     # 서버에 등록된 전체 칭호 목록
/honor info <ID>
```

설정은 `plugins/BrockenreichLand/honors.yml` 에 저장됩니다.

## 닉네임(고정닉) 시스템

마인크래프트 계정 이름 대신 원하는 표시 이름을 채팅/탭리스트에 쓸 수 있는 기능입니다. **머리 위 이름표는
예외** - 마인크래프트 자체 한계로 실제 계정 이름이 계속 보입니다 (위 칭호 시스템의 "머리 위 이름표 제약"
참고).

- 누구나 `/nickname <새 닉네임>` 으로 스스로 설정 가능 (공백 없이 한 단어, 2~16자, `&` 색상 코드 사용 가능).
- 이미 다른 플레이어가 쓰고 있는 닉네임(대소문자 무관 동일 문자열)은 설정할 수 없습니다.
- `/nickname reset` 으로 본인이 언제든 원래 계정 이름으로 되돌릴 수 있습니다.
- OP는 `/nickname <플레이어 원래 이름> reset` / `/nickname <플레이어 원래 이름> <새 닉네임>` 으로 다른
  플레이어의 닉네임을 직접 초기화하거나 강제로 설정할 수 있습니다 (대상은 항상 실제 계정 이름으로 지정).

### 명령어

```
/nickname <새 닉네임>
/nickname reset
/nickname <플레이어> reset        # OP 전용
/nickname <플레이어> <새 닉네임>  # OP 전용
```

설정은 `plugins/BrockenreichLand/nicknames.yml` 에 저장됩니다.

## 화폐(Economy) 시스템

서버 화폐 단위는 **`ȼ`** 이고, 플레이어별 잔액을 저장/이동하는 기능입니다. 소수점 없이 정수 단위로만
다루고, 잔액은 0 밑으로 내려가지 않습니다 (부족하면 송금/회수 자체가 실패).

### 명령어

```
/money                          # 내 잔액 확인
/money balance <플레이어>       # 다른 플레이어 잔액 확인 (누구나 가능)
/money pay <플레이어> <금액>    # 상대에게 송금 (잔액 부족 시 실패)
/money give <플레이어> <금액>   # OP 전용, 지급 (화폐 발행)
/money take <플레이어> <금액>   # OP 전용, 회수 (대상 잔액 부족 시 실패)
/money set <플레이어> <금액>    # OP 전용, 잔액을 특정 값으로 설정
/money top [순위 개수]          # 잔액 순위 (기본 10명, 최대 50명)
```

설정은 `plugins/BrockenreichLand/economy.yml` 에 저장됩니다.

## 농장(Farm) 시스템

이 서버는 `randomTickSpeed`를 0으로 설정해서 운영합니다. 그래서 작물이 자연적인 랜덤 틱으로는 절대 자라지 않고,
대신 `/farm animate`로 인챈트한 씨앗/모종만 심을 수 있으며, 심은 뒤 정해진 고정 시간(실제 시간 기준, 서버를
재시작해도 유지됨)이 지나면 한 번에 완전히 다 자란 상태가 됩니다.

### 바닐라 대비 변경 사항

- 경작지(farmland)를 캐면 흙이 아니라 경작지 자체가 드롭됩니다 (호미 없이 바로 재설치 가능).
- 경작지를 설치하면 바닐라 기본값(마름)이 아니라 항상 최대로 젖어있는 상태(어두운 색 텍스처)로 설치됩니다.
- 경작지를 밟아도 흙으로 돌아가지 않습니다.
- 경작지 위에 블록을 설치하거나, 수분이 말라도 흙으로 돌아가지 않습니다.
- 일반(인챈트되지 않은) 씨앗/모종/열매는 아예 심어지지 않습니다. 심어질 수 있는 위치(경작지/흙/모래/정글 원목 옆면 등)는
  바닐라 규칙을 그대로 따르며, 대상 아이템이 전부 경작지에 심어지는 건 아닙니다 (나무 묘목·달콤한 열매는 흙/잔디 등에 심음).
- 인챈트된 사탕수수는 물가가 아니어도 아무 곳에나 심을 수 있고, 물이 없어도 뽑히지 않습니다 (그 외 배치 조건은 동일).

### `/farm animate`

인챈트하고 싶은 아이템을 손에 들고 입력하면, 손에 든 아이템이 인챈트된 상태(반짝임 효과가 붙음)로 바뀝니다.
이름은 원래 아이템 이름(예: 빨간 버섯/갈색 버섯, 오크 묘목/정글 묘목 등 구체적인 이름) 그대로 유지됩니다.
인챈트된 아이템만 심을 수 있습니다.

대상 아이템: 달콤한 열매, 감자, 당근, 비트 씨앗, 수박 씨앗, 밀 씨앗, 네더 와트, 호박 씨앗, 버섯(빨간/갈색 버섯,
진홍빛/뒤틀린 균), 나무 묘목(오크/가문비/자작/정글/아카시아/짙은참나무/벚나무/맹그로브 순묘), 진달래(꽃 핀 진달래 포함), 코코아,
선인장, 사탕수수, 죽순.

수확한 씨앗은 인챈트 상태가 아니므로, 다시 심으려면 매번 `/farm animate`로 다시 인챈트해야 합니다.

### 고정 성장 시간

심은 뒤 다음 기본 시간이 지나면 자동으로 완전히 다 자랍니다 (`plugins/BrockenreichLand/config.yml`의
`farm.growthMinutes`에서 분 단위로 조정 가능, 변경 후 서버 재시작 필요):

| 작물 | 기본 시간 |
|---|---|
| 달콤한 열매 | 1분 |
| 감자 | 4분 |
| 당근 | 10분 |
| 비트 씨앗 | 15분 |
| 수박 씨앗 | 30분 |
| 호박 씨앗 | 1시간 |
| 밀 씨앗 | 1시간 |
| 네더 와트 | 30분 |
| 버섯 (빨간/갈색 버섯, 진홍빛/뒤틀린 균) | 2시간 |
| 나무 묘목 (정글·짙은 참나무 제외) / 진달래(꽃 핀 진달래 포함) | 3시간 |
| 정글 나무 묘목 | 36시간 |
| 정글 큰 나무 | 168시간 |
| 짙은 참나무 묘목 | 8시간 |
| 코코아 | 1시간 |
| 선인장 | 1시간마다 한 칸씩 |
| 죽순 | 2시간마다 한 칸씩 |
| 사탕수수 | 3시간마다 한 칸씩 |

선인장/죽순/사탕수수 3종은 나머지와 달리 "완전히 다 자람"이 없고, 위 시간마다 계속 한 칸씩 위로 자랍니다
(선인장/사탕수수는 바닐라와 같은 최대 3칸, 죽순은 최대 16칸까지 - 이미 그 높이에 도달했거나 바로 위 칸이
막혀있으면 그 주기엔 그냥 안 자라고 다음 주기에 다시 시도합니다). 이 3종은 한 사람이 아니라 서버의 공용
자원으로 취급되어서, 맨 처음 심은 밑동 블록은 OP만 부술 수 있습니다 (일반 플레이어는 위쪽 칸들만 캐서
수확할 수 있고, 밑동은 그대로 남아 계속 자람). 뼛가루로 수동으로 키우는 것도 OP만 가능하고, 일반 플레이어가
뼛가루를 써도 아무 효과가 없습니다 - 오직 이 고정 주기로만 자랍니다.

나무 묘목은 정글·짙은 참나무를 제외한 오크/가문비/자작/아카시아/벚나무/맹그로브 순묘와 진달래(꽃 핀 진달래 포함)가
"나무 묘목" 시간을 쓰고, 정글과 짙은 참나무는 따로 시간이 있습니다. 정글은 혼자 심으면 36시간이 걸리지만, 정글
나무 묘목 4그루를 2×2로 붙여 심으면 그 순간부터 4그루 모두 168시간짜리 "큰 나무" 타이머로 다시 설정됩니다
(이미 혼자 심어져서 시간이 흐르고 있던 것도 2×2가 완성되면 168시간으로 새로 시작). 짙은 참나무는 바닐라에서도
혼자서는 절대 자라지 않고 2×2로만 자라기 때문에, 4그루가 모두 심어져 2×2가 완성되기 전까지는 성장 타이머
자체가 시작되지 않고 그냥 심어진 채로 있다가, 2×2가 완성되는 순간 4그루 전부 8시간 타이머가 시작됩니다.

2×2로 심어져 큰 나무로 묶인 4그루는 그중 하나만 다 자라기 전에 부숴도 나머지 3그루가 자동으로 같이
부서집니다 (한 그루가 없으면 어차피 나머지 3그루만으로는 큰 나무가 될 수 없기 때문) - 부서진 나머지
그루들도 각각 아이템을 드롭합니다.

심어져서 아직 자라는 중인 작물 블록에 커서를 대고 쉬프트+우클릭하면, 그 작물 1.5칸 위에 이름표처럼 남은 시간
텍스트가 떠서 실시간으로 줄어듭니다. 떠 있는 상태에서 같은 블록에 다시 쉬프트+우클릭하면 사라지고, 따로
끄지 않아도 뜬 지 1분이 지나면 자동으로 사라집니다. 다 자라기 전에 그 블록을 부수면 카운트다운과 (띄워져
있었다면) 이름표도 즉시 같이 정리됩니다.

표시 형식은 남은 시간에 따라 상위 두 단위만 보여줍니다 (예: "3일 4시간", "2시간 15분", "10분 30초", 1분
미만이면 "45초"만).

성장에 따라 텍스처가 바뀌는 작물(밀/당근/감자/비트/네더 와트/코코아/달콤한 열매/수박·호박 씨앗)은 경과 시간에
비례해서 실제 텍스처(나이 단계)가 계속 바뀌며 자랍니다 — 예를 들어 수박·호박 줄기는 바닐라 기준 8단계(0~7)라
그 8단계 전부를 시간에 맞춰 순서대로 거치고, 비트/네더 와트/달콤한 열매처럼 4단계(0~3)뿐인 작물은 그만큼만
거칩니다 (버섯/나무 묘목은 단계별 텍스처가 따로 없는 종류라 심은 모습 그대로 있다가 다 자랄 때 한 번에 바뀝니다).
코코아는 심을 때 클릭한 정글 원목 방향이 자라는 내내 그대로 유지됩니다. 다 자라는 순간에는 뼛가루를 쓴 것과 같은
초록색 반짝임 파티클이 터집니다.

나무 묘목/버섯류(빨간·갈색 버섯, 진홍빛·뒤틀린 균)는 시간이 다 됐어도 자랄 공간이 막혀 있으면(나무는 원목이
아닌 다른 블록에 가로막힌 경우, 버섯은 원목 여부와 상관없이 어떤 블록이든 있으면) 실제로 자라지 못할 수 있는데,
이때는 초록색 파티클 대신 검은 연기 파티클이 터지고, 다시 시도하지 않고 그 자리에 심어진 묘목/버섯 그대로
영구히 남습니다 (나중에 막힌 걸 치워도 자동으로 자라지 않으며, 다시 자라게 하려면 캐고 새로 심어야 합니다).

수박/호박은 다 자라면 줄기가 있던 그 자리가 수박/호박 블록으로 직접 바뀝니다 (바닐라처럼 옆 칸에 별도로 열매가
생기지 않음) — 그래서 줄기가 남아 계속 자라려고 하는 일이 없습니다.

달콤한 열매(sweet berry bush)만 예외적으로, 다 자란 뒤 우클릭으로 열매를 수확해도 나무가 사라지지 않고 그
자리에서 다시 고정 시간이 지나면 또 열매가 맺힙니다 — 나머지 작물과 달리 계속 반복 수확이 가능합니다.

### 테스트용 성장 시간 단축

```
/farm time test        # 지금부터 심는 모든 작물의 성장 시간을 10초로 설정 (관리자 전용)
/farm time test 30     # 초를 직접 지정 (예: 30초)
/farm time default     # config.yml에 설정된 기본 시간으로 되돌림 (관리자 전용)
```
이미 심어진 작물의 남은 시간에는 영향을 주지 않고, 명령 실행 이후 새로 심는 작물부터 적용됩니다.
