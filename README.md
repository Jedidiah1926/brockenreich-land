...
심어져서 아직 자라는 중인 작물 블록에 커서를 대고 쉬프트+우클릭하면, 그 작물 1.5칸 위에 이름표처럼 "○○분
○○초" 텍스트가 떠서 실시간으로 초가 줄어듭니다. 떠 있는 상태에서 같은 블록에 다시 쉬프트+우클릭하면 사라집니다.
다 자라기 전에 그 블록을 부수면 카운트다운과 (띄워져 있었다면) 이름표도 즉시 같이 정리됩니다.
