...
- 일반(인챈트되지 않은) 씨앗/모종/열매는 아예 심어지지 않습니다. 심어질 수 있는 위치(경작지/흙/모래/정글 원목 옆면 등)는
  바닐라 규칙을 그대로 따르며, 14종이 전부 경작지에 심어지는 건 아닙니다 (나무 묘목·달콤한 열매는 흙/잔디 등에 심음).
- 인챈트된 사탕수수는 물가가 아니어도 아무 곳에나 심을 수 있고, 물이 없어도 뽑히지 않습니다 (그 외 배치 조건은 동일).

### `/farm animate`

인챈트하고 싶은 아이템을 손에 들고 입력하면, 손에 든 아이템이 인챈트된 상태(이름이 하늘색으로 바뀌고
반짝임 효과가 붙음)로 바뀝니다. 인챈트된 아이템만 심을 수 있습니다.

대상 아이템 14종: 달콤한 열매, 감자, 당근, 비트 씨앗, 수박 씨앗, 밀 씨앗, 네더 와트, 호박 씨앗, 버섯(빨간/갈색),
나무 묘목(오크/가문비/자작/정글/아카시아/짙은참나무/벚나무/맹그로브 순묘), 코코아, 선인장, 사탕수수, 죽순.
...
있습니다 (성장은 바닐라처럼 뼛가루 등에 맡김). 나머지 11종은 심은 뒤 다음 기본 시간이 지나면 자동으로
완전히 다 자랍니다 (`plugins/BrockenreichLand/config.yml`의 `farm.growthMinutes`에서 분 단위로 조정 가능,
변경 후 서버 재시작 필요):
...
심어져서 아직 자라는 중인 작물 블록에 커서를 대고 쉬프트+우클릭하면 남은 시간이 "○○분 ○○초" 형태로
화면 중앙에 크게 표시됩니다.
