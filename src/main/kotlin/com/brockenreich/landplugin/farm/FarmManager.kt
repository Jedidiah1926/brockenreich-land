 ...
 * saplings, which don't have a texture-bearing age) is visibly stepped through 4 growth stages
 * (0, 1/3, 2/3, fully grown) at the matching elapsed-time fractions, instead of sitting frozen
 * until the single instant it finishes.
 */
class FarmManager(private val plugin: Plugin, private val dataFolder: File) {
    ...
    private data class Entry(val type: FarmCropType, val plantedAt: Long, val dueAt: Long, var stage: Int)
    ...
    fun plant(block: Block, type: FarmCropType) {
        val duration = growthMillis[type] ?: return
        val now = System.currentTimeMillis()
        entries[keyOf(block)] = Entry(type, now, now + duration, 0)
        save()
    }
    ...
    private fun tick() {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.dueAt <= now) {
                ...
                // Don't force a chunk load just to finish a timer - it'll be picked up on a later
            ...
            val targetStage = stageFor(now, entry)
            if (targetStage > entry.stage) {
                val world = Bukkit.getWorld(key.world) ?: continue
                if (!world.isChunkLoaded(key.x shr 4, key.z shr 4)) continue
                advanceStage(world.getBlockAt(key.x, key.y, key.z), targetStage)
    ...
    /** Steps an Ageable crop's age to match [stage] (1 or 2 - stage 0/3 are handled by planting/forceFullyGrown). */
    private fun advanceStage(block: Block, stage: Int) {
        val data = block.blockData as? Ageable ?: return
        val age = when (stage) {
            1 -> data.maximumAge / 3
            2 -> data.maximumAge * 2 / 3
            else -> data.maximumAge
        }
        if (age <= data.age) return
        data.age = age
    ...
    private fun isStillGrowing(block: Block, type: FarmCropType): Boolean = when (type) {
        ...
        FarmCropType.MELON -> hasNoAdjacentFruit(block, Material.MELON)
        FarmCropType.PUMPKIN -> hasNoAdjacentFruit(block, Material.PUMPKIN)
        else -> (block.blockData as? Ageable)?.let { it.age < it.maximumAge } ?: false
    }

    private fun hasNoAdjacentFruit(block: Block, fruit: Material): Boolean {
        val faces = arrayOf(BlockFace.NORTH, BlockFace.SOUTH, BlockFace.EAST, BlockFace.WEST)
        return faces.none { block.getRelative(it).type == fruit }
    }

    // Repeatedly simulating bonemeal (rather than hand-setting block data) reuses vanilla's own
    // growth logic for every crop shape, including odd ones like a melon stem's fruit placement
    // or a sapling's tree generation, instead of reimplementing each one's rules by hand. Most
    // crops finish in 1-2 calls; the probabilistic ones (mushroom/sapling/fruit spawn) are looped
    // up to 40 times so a bad string of rolls essentially never leaves growth incomplete.
    ...
    fun load() {
        ...
        yaml.getConfigurationSection("entries")?.getKeys(false)?.forEach { id ->
            ...
            val stage = section.getInt("stage")
            entries[key] = Entry(type, plantedAt, dueAt, stage)
        }
    }
    ...
    fun save() {
        ...
        entries.entries.forEachIndexed { index, (key, entry) ->
            ...
            yaml.set("$base.stage", entry.stage)
        }
        dataFolder.mkdirs()
        yaml.save(file)
    }
